    ...
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6 py-12">
        ...
        <div>
          ...
          <Link to="/add-child">
            <button className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 ml-4 rounded-md">
              ➕ Register Child
            </button>
          </Link>
        </div>
      ...
      <div className="flex mb-4 border-b">
        <button
          className={`px-4 py-2 ${activeTab === "program" ? "border-b-2 border-orange-500 font-semibold" : "text-gray-600"}`}
          onClick={() => setActiveTab("program")}
        >
          Program Roster
        </button>
        <button
          className={`px-4 py-2 ${activeTab === "cleaning" ? "border-b-2 border-orange-500 font-semibold" : "text-gray-600"}`}
          onClick={() => setActiveTab("cleaning")}
        >
          Cleaning Roster
        </button>
        <button
          className={`px-4 py-2 ${activeTab === "teachers" ? "border-b-2 border-orange-500 font-semibold" : "text-gray-600"}`}
      ...
      {/* Content */}
      {/* 2. LOGIC FIX: We moved TeachersRoster so it shows even if children.length is 0 */}
      <div>
        {activeTab === "program" && (
          children.length === 0 ? <p className="text-gray-500 text-center py-10">No children registered yet.</p> : <ProgramRoster roster={programRoster} />
        )}
        
        {activeTab === "cleaning" && (
          children.length === 0 ? <p className="text-gray-500 text-center py-10">No children registered yet.</p> : <CleaningRoster roster={cleaningRoster} />
        )}
